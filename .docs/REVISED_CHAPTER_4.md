...
The Analytics screen presents visual breakdowns of the user's spending and income data through four chart sections. At the top, time range filter chips allow the user to switch the dataset between predefined periods. Below the filter, a row of four summary metric cards displays total income, total expenses, net savings, and the highest-spending category for the selected range. The four chart sections that follow present a spending trend line chart showing daily expenditure over time, an income versus expense grouped bar chart comparing monthly totals, a tappable category pie chart showing the proportional distribution of expense categories, and a budget comparison chart contrasting allocated and actual spending per category. Each chart section is wrapped in a card with a labelled header. Empty state messages are shown in place of each chart when no transaction data exists for the selected period.

#### Reports Screen

The Reports screen provides financial summary data and document export functionality. A row of filter chips at the top allows the user to select a reporting period, with options covering the current month, previous month, and longer historical ranges. The screen displays three report panels: a spending summary, a budget utilisation report, and a category-level breakdown. Two export options are accessible via buttons: CSV export and PDF export. CSV export produces either a raw transaction list or a spending summary file, named with the selected period and the current date. PDF export produces a formatted spending report or budget report document. After a successful export, a confirmation dialog presents the output file path and file size, and an operating system share sheet is available to allow the user to send the file via email, cloud storage, or messaging applications.

#### Add Goal Screen

The Add Goal screen collects the information required to create or edit a savings goal and is used for both creation and editing depending on the context in which it is opened. The form contains a goal name field, an optional description field, a target amount field prefixed with the RM currency symbol, and a deadline date picker restricted to future dates. A scrollable chip selector presents goal categories including Emergency Fund, Travel, Home, Education, Vehicle, Health, Retirement, Business, Investment, and Other, each paired with a recognisable icon. A three-option toggle bar allows the user to set the goal priority as Low, Medium, or High. When the screen is opened in edit mode, a current amount field is also shown, allowing the user to update progress toward the goal manually. On successful submission, the screen returns to the Goals screen with the updated goal list.
