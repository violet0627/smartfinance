...
The streak system tracks consecutive days on which the user records at least one financial activity. The `HabitStreak` model stores `CurrentStreak`, `LongestStreak`, and `LastActivityDate`. Each time the gamification check endpoint is called (triggered after each transaction save), the backend compares `LastActivityDate` with today's date: if it was yesterday, the streak increments; if it was today, the streak is unchanged; if it was more than one day ago, the streak resets to one.
...
**Usability Testing** evaluates the ease with which target users (Malaysian young adults with varying financial literacy) can accomplish representative tasks. Participants are recruited from the target demographic and asked to complete six predefined tasks without guidance. Evaluators record task completion rates, time-on-task, error counts, and collect post-session questionnaire responses using a SUS-inspired Likert scale instrument. This strategy validates the user experience design decisions made during the prototyping phase.
