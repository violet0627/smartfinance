...
On the backend, these numbers are calculated by summing all income and expense transactions for the current month from the database, filtered by the logged-in user's ID.
...
On the backend, each goal stores two values — the target amount and the current amount saved.

The progress bar is calculated by dividing the current amount by the target amount, then multiplying by 100 to get a percentage.

When the user adds a contribution, the backend updates the current amount and recalculates the percentage.

When the goal reaches 100%, it's automatically marked as **completed** and the user earns a gamification achievement.
...
The profit or loss for each holding is calculated as — current price minus purchase price, multiplied by quantity.

The overall portfolio gain or loss at the top is the sum of all individual holdings.
...
The total cost is calculated automatically — quantity times purchase price.

Users update the current price manually over time, and the backend recalculates the gain or loss immediately.
...
They can download a **CSV** — the Flask backend queries the database, formats the transaction data into rows and columns, and returns it as a downloadable file that opens in Excel.

Or a **PDF** — this one is generated entirely on the Flutter side using a PDF library, then shared via the system share sheet without hitting the server at all.
...
On the backend, every time a user completes an action, the server checks which achievements that action applies to and updates the progress automatically.

When progress hits 100%, the achievement is unlocked and the XP reward is added to the user's total.

XP fills a level bar — from Level 1 to Level 10.

There's also a **habit streak** — the backend checks if the user has recorded at least one transaction today.

If yes, the streak count increases. Miss a day and the streak resets to zero.

And a **leaderboard** to compare XP rankings with other users — each user can choose to show or hide themselves from it in Settings.
