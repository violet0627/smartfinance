...
import '../insights/financial_insights_screen.dart';
...
class _DashboardScreenState extends State<DashboardScreen> {
  ...
  double? _lastMonthExpense;
  String? _avatarPath;

  // Preserves scroll position when returning from sub-screens; without this
  // every silent _loadData() call jumps back to the top of the page.
  final ScrollController _scrollController = ScrollController();

  // JSON numbers from the API can be int, double, or String — this handles all cases.
  ...
  // silent: true = refresh in the background without showing the skeleton.
  // Used when returning from sub-screens to preserve the scroll position.
  Future<void> _loadData({bool silent = false}) async {
    ...
    try {
      ...
      setState(() {
        ...
        // Only use the saved path if the file still exists — user may have cleared app storage.
        _avatarPath = (savedPath != null && File(savedPath).existsSync()) ? savedPath : null;
      });

      try {
        final transactionsResult = await ApiService.getUserTransactions(userId, limit: 5);
        if (transactionsResult['success']) {
          final transactionsList = transactionsResult['transactions'] as List;
          setState(() {
            _recentTransactions = transactionsList.map((json) => TransactionModel.fromJson(json)).toList();
          });
        }
      } catch (e) {
      ...
      try {
        ...
        if (summaryResults[0]['success']) setState(() => _summary = summaryResults[0]);
        if (summaryResults[1]['success']) {
          setState(() => _lastMonthExpense = _toDouble(summaryResults[1]['totalExpense']));
        }
      } catch (e) {
      ...
      try {
        ...
        if (portfolioResult['success']) {
          ...
          setState(() => _portfolio = portfolioData != null ? PortfolioSummary.fromJson(portfolioData) : null);
        }
      } catch (e) {
      ...
      try {
        ...
        if (statsResult['success'] && statsResult['stats'] != null) {
          setState(() => _userStats = UserStats.fromJson(statsResult['stats']));
        }
      } catch (e) {
      ...
      try {
        ...
        if (goalsResult['success']) setState(() => _goalsSummary = goalsResult['summary']);
      } catch (e) {
      ...
      try {
        ...
        if (recurringResult['success']) {
          ...
          setState(() => _upcomingBills = upcomingReminders);
        }
      } catch (e) {
      ...
      body: _isLoading
          ? SingleChildScrollView(
              ...
              child: Column(
                ...
                children: [
                  SkeletonBox(width: 200, height: 24, borderRadius: BorderRadius.circular(4)),
                        ...
                        Expanded(
                          child: QuickActionCard(
                            ...
                            onTap: () {
                              Navigator.push(
                                context,
                                MaterialPageRoute(builder: (_) => const PortfolioOverviewScreen()),
                              ).then((_) => _loadData(silent: true));
                            },
                          ),
                        ),
        ...
        child: Column(
          ...
          children: [
            ...
            Row(
              ...
              children: [
                Column(
                  crossAxisAlignment: CrossAxisAlignment.start,
                  children: [
                    Text('Spent', style: TextStyle(color: AppColors.textSecondary, fontSize: 12)),
                    Text(
                ...
                Column(
                  ...
                  children: [
                    Text('Remaining', style: TextStyle(color: AppColors.textSecondary, fontSize: 12)),
                    Text(
            ...
            Row(
              mainAxisAlignment: MainAxisAlignment.spaceBetween,
              children: [
                Text('RM ${budget.totalBudget.toStringAsFixed(2)} total',
                    style: TextStyle(color: AppColors.textSecondary, fontSize: 12)),
                Text('${budget.daysLeftInMonth} days left',
                    style: TextStyle(color: AppColors.textSecondary, fontSize: 12)),
              ],
            ),
              ...
              Container(
                ...
                child: Row(
                  children: [
                    ...
                    Expanded(
                      child: Text(
                        // totalRemaining is negative when over budget; negate to show a positive overspend figure.
      ...
      child: Container(
        ...
        child: Column(
          ...
          children: [
            ...
            const SizedBox(height: 16),
            LevelProgressWidget(stats: _userStats!, compact: true),
          ],
        ),
      ),
    );
