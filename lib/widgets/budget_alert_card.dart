  ...
  List<BudgetAlert> _generateAlerts() {
    ...
    if (budget.isOverBudget) {
      alerts.add(BudgetAlert(
        ...
        message: 'You\'ve overspent by RM ${(-budget.totalRemaining).toStringAsFixed(2)}. Consider reducing expenses.',
      ));
      ...
      alerts.add(BudgetAlert(
        ...
        message: 'You\'ve used ${percentage.toStringAsFixed(0)}% of your budget. Only RM ${budget.totalRemaining.toStringAsFixed(2)} remaining.',
      ));
      ...
      alerts.add(BudgetAlert(
        ...
        icon: Icons.info,
        title: 'Budget Alert',
        message: '${percentage.toStringAsFixed(0)}% of your budget used. You\'re on track but watch your spending.',
      ));
    }

    final overBudgetCategories = budget.categories.where((cat) => cat.isOverBudget).toList();
    if (overBudgetCategories.isNotEmpty) {
      final categoryNames = overBudgetCategories.map((cat) => cat.categoryName).join(', ');
      alerts.add(BudgetAlert(
        type: AlertType.danger,
        icon: Icons.category,
    ...
    if (warningCategories.isNotEmpty) {
      final categoryNames = warningCategories.map((cat) => cat.categoryName).join(', ');
      alerts.add(BudgetAlert(
        type: AlertType.warning,
    ...
    if (budget.daysLeftInMonth <= 3 && !budget.isOverBudget) {
      ...
      alerts.add(BudgetAlert(
        ...
        message: 'Only ${budget.daysLeftInMonth} days left. Daily budget: RM ${dailyBudget.toStringAsFixed(2)}.',
      ));
    }
