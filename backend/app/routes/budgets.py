...
def create_budget():
    try:
        ...
        if abs(category_total - float(data['totalBudget'])) > 0.01:  # float tolerance
        ...
        db.session.flush()  # assigns BudgetId before commit so categories can reference it
...
def get_current_budget(user_id):
    try:
        ...
        budget = Budget.query.filter_by(UserId=user_id, MonthYear=current_month).first()
...
def _update_budget_spending(budget):
    try:
        ...
        # timedelta has no months support — advance manually; handle December wraparound
        if month == '12':
        ...
        for budget_cat in budget.categories:
            budget_cat.SpentAmount = spending_by_category.get(budget_cat.CategoryName, 0)

        db.session.commit()
    except Exception as e:
        db.session.rollback()
        raise e  # re-raise so the calling route handles the response
